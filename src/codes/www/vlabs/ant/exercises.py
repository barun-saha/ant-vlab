...
def ns2_submit(request):
    ...
    if request.method == 'POST':
        ...
        if trace_file_exists:
            ...
            request.session[request.session.session_key] = {NS_TRACEFILE_KEY : trace_file_name}
            #print request.session.get(request.session.session_key)

            # Convert physical path of trace file to URL
            output['trace'] = trace_file_name.replace(r'/var/', '/ant/v_media/')
        ...
        output = {'mesg': 'Invalid attempt to access a resource!'}
