...
YAHOO.lang.extend(WireIt.Wire, WireIt.CanvasElement, {
  ...
  // Added by Barun 28-Jun-2010
  // Square type of arrows for representing loops (but not restricted to) in CFG

  /* Dashed arrows for Use Case dia */
  drawDashedArrows: function() {
      var d = 7; // arrow width/2
      var redim = d+3; //we have to make the canvas a little bigger because of arrows
      var margin=[4+redim,4+redim];
...
//	  (e2) .-------	* (p2)
   ...
   drawRightSquareArrows: function()
   {	  
      // Get the positions of the terminals
      var p1 = this.terminal1.getXY();
      var p2 = this.terminal2.getXY();
...
//	  (p2) .<------	* (e2)
    ...
    */
   redraw: function() {
	   // Added by Barun - Jun-28-2010
	   // Add two new types of wires: leftSquareArrows and rightSquareArrows
	   this.options.drawingMethod = document.getElementById('hEdgeType').value
	   // End
      ...
      if(this.options.drawingMethod == 'straight') {
       ...
       // Added by Barun - Jun-28-2010
       // Add two new types of wires: leftSquareArrows and rightSquareArrows
      else if (this.options.drawingMethod == 'leftSquareArrows') {
            this.drawLeftSquareArrows();
      }
      else if (this.options.drawingMethod == 'rightSquareArrows') {
            this.drawRightSquareArrows();
      }
      else if (this.options.drawingMethod == 'dashedArrows') {
            this.drawDashedArrows();
      }
       // End
      else {
