    ...
    */
    WireIt.WiringEditor = function(options) {
        ...
        */
        ...
        // Right accordion
        //this.renderAccordion(); -- Commented by Barun
    ...
    WireIt.WiringEditor.prototype = {
        ...
        */
        setOptions: function(options) {
            ...
            */
            ...
            // Modified by Barun 30-Jun-2010
            this.adapter = WireIt.WiringEditor.adapters.MyAdapter;  // options.adapter || WireIt.WiringEditor.adapters.JsonRpc;
		...
		{"type": "text", inputParams: {"name": "description", label: "Description", cols: 30, rows: 4}}
	      ...
	      {position: 'left', width: 100, resize: true, body: 'left', gutter: '5px', collapse: true,
	          collapseSize: 25, header: 'Modules', scroll: true, animate: true
	      },
	      {position: 'center', body: 'center', gutter: '5px'},
	      {position: 'right', width: 2, resize: false, body: 'right', gutter: '0px', collapse: true,
	          collapseSize: 0, /*header: 'Properties', scroll: true,*/animate: true
	      } /* width: 320, resize: true, gutter: '5px', collapseSize: 25 */
	   ]
            };
        ...
        */
        saveModule: function() {
            ...
            /* Commented by Barun : 07-Aug-2010
            if (value.name === "") {
            ...
            */
            ...
            // Modified by Barun 30-Jun-2010, 02-Jul-2010
            //this.tempSavedWiring = { name: value.name, working: JSON.stringify(value.working), language: this.options.languageName };
            this.tempSavedWiring = {nodes: JSON.stringify(value.working.modules), edges: JSON.stringify(value.working.wires)};
            // TODO:
            //this.tempSavedWiring = { nodes: value.working.modules, edges: value.working.wires };            

            var strGraph = JSON.stringify(this.tempSavedWiring);            
            document.getElementById("divJSONGraph").innerHTML = strGraph;                        
            this.alert("Save: " + document.getElementById("divJSONGraph").innerHTML);

            //var gObj = JSON.parse(strGraph);
            //var nodes = JSON.parse(gObj.nodes);
            //alert(JSON.stringify(nodes));
            //alert(typeof (nodes));
            //            var n1 = JSON.parse(nodes[0]);
            //            var n2 = JSON.parse(nodes[1]);
            //            var c1 = JSON.parse(n1.config);
            //            var c2 = JSON.parse(n2.config);
            //            alert(c1 + " | " + c2);
            //            alert(JSON.stringify(n1) + "|" + JSON.stringify(n2));
            // End
        ...
        */
        getValue: function() {
            ...
            for (i = 0; i < this.layer.containers.length; i++) {
                // Modified by Barun 30-Jun-2010
                // Original
                //obj.modules.push({ name: this.layer.containers[i].options.title, label: this.layer.containers[i].labelField.getValue(), value: this.layer.containers[i].getValue(), config: this.layer.containers[i].getConfig() });
                // Removed title, value; added labelField value
                // Barun, 30-Jun-2010
                // config now contains only xtype, not position
                // Also contains node category, 27-Aug-2010
                //this.alert(JSON.stringify(this.layer.containers[i].getConfig()));
                var customConfig = {"xtype": this.layer.containers[i].getConfig().xtype, "category": this.layer.containers[i].getConfig().category};
                obj.modules.push({label: this.layer.containers[i].labelField.getValue().toLowerCase(), config: customConfig});
                // End
            }
            ...
            for (i = 0; i < this.layer.wires.length; i++) {
                ...
                // Modified by Barun 03-Jul-2010
                // Wires will contain only moduleID; terminal discarded, since user can connect two same nodes but on diff terminals
                // Original
                ...
                // Modified below
                // 29-Jul-2010: Store the node label; this will reflect the correct
                // connectivity among nodes irrespective of the sequence of their creation
                var wireObj = {
                    src: {moduleId: wire.terminal1.container.labelField.value.toString().toLowerCase()},
                    tgt: {moduleId: wire.terminal2.container.labelField.value.toString().toLowerCase()}
                };
                // 29-Jul-2010
                // End
